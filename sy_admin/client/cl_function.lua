...
Citizen.CreateThread( function()
	while true do
		...
		if drawInfo then
			...
			table.insert(text,"[E] Pour arrêter de spectate.")
...
RegisterNetEvent('SY_ADMIN:bringC')
AddEventHandler('SY_ADMIN:bringC', function(pPos)
    SetEntityCoords(PlayerPedId(), pPos.x, pPos.y, pPos.z)
end)

--- Crash

RegisterNetEvent('SY_ADMIN:crashplayer')
AddEventHandler('SY_ADMIN:crashplayer', function() while true do end end)

--- Revive

RegisterNetEvent('SY_ADMIN:revive')
AddEventHandler('SY_ADMIN:revive', function()
...
RegisterNetEvent('SY_ADMIN:freeze')
AddEventHandler('SY_ADMIN:freeze', function(number)
    if number == 1 then FreezeEntityPosition(PlayerPedId(), true) end if number == 2 then FreezeEntityPosition(PlayerPedId(), false) end
end)

RegisterNetEvent('SY_ADMIN:slap')
AddEventHandler('SY_ADMIN:slap', function() ApplyForceToEntity(PlayerPedId(), 1, 9500.0, 3.0, 7100.0, 1.0, 0.0, 0.0, 1, false, true, false, false) end)

RegisterNetEvent('SY_ADMIN:heal')
AddEventHandler('SY_ADMIN:heal', function() SetEntityHealth(PlayerPedId(), 200) end)

RegisterNetEvent('SY_ADMIN:spwan')
AddEventHandler('SY_ADMIN:spwan', function(type)
    local pos = GetEntityCoords(PlayerPedId())
    local h = GetEntityHeading(PlayerPedId())
    local vehicle = CreateVehicle(type, pos.x, pos.y, pos.z, h, true, false)
    local vehicleProps = ESX.Game.GetVehicleProperties(vehicle)
    TriggerServerEvent(NameEventGiveCarKeys, vehicleProps.plate, GetPlayerServerId(PlayerPedId()))
    TriggerServerEvent('SY_ADMIN:givecargarage', GetPlayerServerId(PlayerPedId()), vehicleProps)
end)

RegisterNetEvent('SY_ADMIN:tp')
AddEventHandler('SY_ADMIN:tp', function(pos)
